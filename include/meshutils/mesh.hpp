...
public:
  ...
  // add a cube from (-1, -1, -1) to (1, 1, 1)
  //
  //   6     7
  // 2 4  3  5
  // 0    1 
  size_t addCube(const glm::mat4 &transform = glm::mat4()) {
    size_t result = indices_.size();
    // front
    glm::vec3 pos[8] = {
      glm::vec3(-1, -1, -1),
    ...
    for (int face = 0; face != 6; ++face) {
      for (int v = 0; v != 4; ++v) {
        addVertexTransformed(transform, pos[cube[face*4+v]], normal[face], uv[v]);
      }
      addIndex(result + face*4 + 0);
      addIndex(result + face*4 + 2);
      addIndex(result + face*4 + 1);
      addIndex(result + face*4 + 1);
      addIndex(result + face*4 + 2);
      addIndex(result + face*4 + 3);
    }
